...
System.out.println("올바른 지점의 코드르르 입력해주세요.");
		...
		for (int i=0; i<moviecenters.size();i++) {
			if (moviecenters.get(i).getMovcenter().equals(movcenter)) {
				...
				switch(num) {
				case 1:
					db.insertTheater(thecode, thename, movcenter, price, cleantime);
					break;
				case 2:
...
public void changetheater() {
	...
	if (!movcenters.contains(movcenter))
		...
		if (!thecodes.contains(thecode))
			...
			switch(num) {
			case 1:
				db.updateTheater(thecode, thename, movcenter, price, cleantime);
				break;
			case 2:
...
public void deletetheater() {
	...
	if (!movcenters.contains(movcenter))
		...
		if (!thecodes.contains(thecode))
			...
			switch(num) {
			case 1:
				db.deleteTheater(movcenter, thecode, thename);
				break;
			case 2:
				System.out.println("상영관 삭제가 취소되었습니다.");
				break;

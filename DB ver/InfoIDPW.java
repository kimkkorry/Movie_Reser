...
public class InfoIDPW {
	public static void infoidpassword() {
		...
		int k = account.size();
		for (int i=0; i<k; i++) {
			Phonenum.add(account.get(i).getPhone());
		...
		while(true) {
			...
			case 1:
				...
				if (Phonenum.contains(phonenum)) {
					for(int i=0; i<k; i++) {
						if(phonenum.equals(account.get(i).getPhone())){
							System.out.println("다음으로 생년월일을 입력해주세요.");
							System.out.println("(Ex : 000501) : ");
							String birth = scanner.next();
							if(account.get(i).getBirth().replaceAll("-", "").contains(birth)) {
				...
				if (Id.contains(id)) {
					for(int i=0; i<k; i++) {
						if(id.equals(account.get(i).getID())){
							...
							if(phonenumber.equals(account.get(i).getPhone())) {
								System.out.println("다음으로 생년월일을 입력해주세요.");
								System.out.println("(Ex : 000501) : ");
								String birth = scanner.next();
								if(account.get(i).getBirth().replaceAll("-", "").contains(birth)) {
									System.out.printf("%s님의 비밀번호는 %s입니다.\n", account.get(i).getUsername(),account.get(i).getUserpw());
								}else {
									System.out.println("등록된 생년월일이 아닙니다.");
								}
							}else{

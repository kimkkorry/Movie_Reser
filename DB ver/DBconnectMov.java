...
theatername, centercode, seatprice, cleantime, totalseats, theatercode);
		stmt.executeUpdate(sql);
		System.out.printf("[%s]의 [%s] 상영관 정보 수정이 완료되었습니다.\n", centername, theatername);
		} catch (Exception e) {
		System.out.println("DB 작업 중 문제 발생: " + e.getMessage());
		e.printStackTrace();
		}
...
moviecode, theatercode, starttime, endtime, soldseats, screencode);
	...
	public  String lookstarttime(String screencode) {
		//================= DB작업 ===============
		try {
			String starttime = "";
			stmt = conn.createStatement();
	...
	public  String lookendtime(String screencode) {
		//================= DB작업 ===============
		try {
			String endtime = "";
			stmt = conn.createStatement();
	...
	public void foreignkeychecks1() {
		//================= DB작업 ===============
		try {
			stmt = conn.createStatement();
			String sql = String.format("SET foreign_key_checks = 1;");
			stmt.executeUpdate(sql);
		}catch (Exception e) {
			System.out.println("DB 작업중 문제 발생: " + e.getMessage());
			e.printStackTrace();
...
public ArrayList<String> jointhmc(String theatercode) {
		
	ArrayList<THEATER> theaters = gettheater();
...
public ArrayList<String> joinscrmov(String screencode) {
	
	ArrayList<SCREEN> screens = getscreen();
	ArrayList<String> AddrList = new ArrayList<>();
	String moviecode = "";
	for (SCREEN screen : screens) {
		if (screen.getScreencode().equals(screencode))
			moviecode = screen.getMoviecode();
	}
...
public ArrayList<String> joinscrthe(String screencode) {
...
public ArrayList<String> joinresthe(String reservenumber) {
	
	ArrayList<RESERVATION> resvervaions = getreservations();
	ArrayList<String> AddrList = new ArrayList<>();
	String theatercode = "";
	for (RESERVATION reservation : resvervaions) {
		if (reservation.getReservenumber().equals(reservenumber))
...
public ArrayList<String> joinresscr(String reservenumber) {
	...
	for (RESERVATION reservation : resvervaions) {
		if (reservation.getReservenumber().equals(reservenumber))
			screencode = reservation.getScreencode();
	}
	String startdate = lookstartdate(screencode);
	String starttime = lookstarttime(screencode);
	String endtime = lookendtime(screencode);
...
public ArrayList<String> joinresmov(String reservenumber) {
	ArrayList<RESERVATION> resvervaions = getreservations();
	ArrayList<String> AddrList = new ArrayList<>();
	String moviecode = "";
	for (RESERVATION reservation : resvervaions) {
		if (reservation.getReservenumber().equals(reservenumber))
			moviecode = reservation.getMoviecode();
	}
	try {
		stmt = conn.createStatement();
